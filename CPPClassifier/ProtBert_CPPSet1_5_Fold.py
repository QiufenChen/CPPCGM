...
for fold, (train_idx, valid_idx) in enumerate(kf.split(seqs, labels)):
    ...
    training_args = TrainingArguments(
        learning_rate=1e-4,
        output_dir=f'results/CPPSet1_ProtBert_fold_{fold + 1}',
        num_train_epochs=20,
        per_device_train_batch_size=8,
        per_device_eval_batch_size=8,
        warmup_steps=10,
        weight_decay=0.01,
        logging_dir=f'logs/CPPSet1_ProtBert_fold_{fold + 1}',
        logging_steps=1,
        do_train=True,
        do_eval=True,
    ...
    file_name = f'model/CPPSet1_ProtBert_fold_{fold + 1}'
    ...
    torch.save(pred_labels, f'results/CPPSet1_ProtBert_pred_babels_fold_{fold + 1}.pt')
    torch.save(true_labels, f'results/CPPSet1_ProtBert_true_babels_fold_{fold + 1}.pt')

# Aggregate results
avg_results = {key: np.mean([fold[key] for fold in fold_results]) for key in fold_results[0].keys()}

file_path = 'results/CPPSet1_ProtBert.json'
